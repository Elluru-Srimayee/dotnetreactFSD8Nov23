...
namespace HotelBookingApi.Services
{
    public class RoomService : IRoomService
    {
        ...
        private readonly IRepository<int, Models.RoomFacilities> _roomFacilitiesRepository;
        private readonly IRepository<int, Reservation> _reservationRepository;

        public RoomService(IRepository<int, Room> repository, IRepository<int, Models.RoomFacilities> roomFacilitiesRepository, IRepository<int, Reservation> reservationRepository)
        {
        ...
        public RoomDTO AddRoom(RoomDTO roomDTO)
        {
            ...
            {
                Models.RoomFacilities roomAmenity = new Models.RoomFacilities()
                {
                    RoomId = id,
                    Facilities = a,
                };
                _roomFacilitiesRepository.Add(roomAmenity);
            }
        ...
        private List<Room> CheckAvailableRooms(List<Room> room, string checkIn, string checkOut)
        {
            ...
            {
                var reservation = (from Reservation in _reservationRepository
            .GetAll()
            .Where(reservation =>
                reservation.RoomId == a.RoomId &&
                (DateTime.Parse(checkIn).Date >= DateTime.Parse(reservation.CheckIn).Date &&
                 DateTime.Parse(checkIn).Date <= DateTime.Parse(reservation.CheckOut).Date ||
                 DateTime.Parse(checkOut).Date <= DateTime.Parse(reservation.CheckOut).Date &&
                 DateTime.Parse(checkOut).Date >= DateTime.Parse(reservation.CheckIn).Date))
                 select Reservation
            )
            .ToList();
                int count = 0;
                foreach (var b in reservation)
                {
                    count += b.TotalRoom;
                }
                a.TotalRooms -= count;
